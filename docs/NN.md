...
- A **library of trained nets per `NetArch`** — the infra ships (`ChampionLibrary` + `scripts/bake-champs.sh`
  + the onboarding/DriverControls arch pick); run the bake to populate all 25 archs (only the default is baked
  so far).
