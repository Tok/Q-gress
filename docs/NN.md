...
- **`GenomeIO` / `NetStore` / `Champion` / `ChampionLibrary`** — JSON encode/decode of genome + arch + fitness;
  `localStorage` persistence (survives reload); a **per-arch baked champion library** (`ChampionGenomes`, one
  genome per `NetArch`, regenerated by `scripts/bake-champs.sh`), with `Champion` the default-arch alias.
...
- A **library of trained nets per `NetArch`** — **shipped**: all 25 archs are baked (`ChampionLibrary` /
  `ChampionGenomes`, regenerated by `scripts/bake-champs.sh`), picked per side in onboarding/DriverControls.
  Each beats the `HeuristicPolicy` baseline on held-out seeds. Next: deeper budget / self-play to strengthen them.
