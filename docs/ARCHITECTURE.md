...
- **`agent/`** — the AI. `Agent` (faction members), `NonFaction` (recruitable NPCs),
  ...
  - **`ai/`** — the AI substrate (PLAN Phase 6.0/6.1): `FactionPolicy` (the per-faction source of slider
    weightings; default `DomSliderPolicy` reads the DOM), `SliderVector` (the 18 sliders as one ordered
...
`SliderVectorPolicy` via `FactionPolicies.set`, re-tuning the 18 sliders at checkpoint cadence).
